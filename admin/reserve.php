<?php
	...
	if($mi_sesion->getValor('id_restaurant')==TRUE){ // Controlamos que la sesión sea la del restaurante
		...
		if(isset($_POST['update'])){
			header ('Location: reserve.php'); // Actualizamos la página
		}
...
<head>
	...
	<title>Reservas - Portal de Administración</title>
</head>
<body>
<!--
/*************************************************************************************************************************/
...
-->
	<?php
		...
		$tag->openTag('div','','menutop','');
			...
			$menu->cargarEnlace('reserve.php','Reservas','','selected');
		...
		$tag->openTag('div','contenedor','margen',''); 
			...
			$tag->openTag('div','contenedor-table','','');
				...
				$input->addInput('submit','update','Actualizar',array('id'=>'submit','class'=>'btn4'));
				$tag->closeTag('div');
				
				// Creamos una tabla para mostrar los detalles de las reservas añadidas por parte del usuario 
				$tag->openTag('table','','board-table',array('border'=>'2'));
					...
					$restaurant->getReservationByCif($restaurant_id); // Obtenemos los datos de las reservas que ha realizado el usuario teniendo en cuenta la ID del restaurante
					...
					if(isset($restaurant->consulta)!=null){ // Siempre y cuando la consulta sea diferente de nulo hacemos lo siguiente
						...
						// Mostramos las reservas de los usuarios en el restaurante determinado
						$tag->openTag('tr','','','');
						
							$tag->openTag('td','','','');
								$tag->output($values['id_reservation']);
							$tag->closeTag('td');
							$tag->openTag('td','','','');
