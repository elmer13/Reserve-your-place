				...
				$tag->openTag("div","contenedor-table","","");
					$form->startForm("","POST","", array('name'=>'frmempleado','class'=>'generic','enctype'=>'multipart/form-data', 'onsubmit'=>'')); // Abrimos el formulario 
						$tag->openTag("div","","title-menu","");
							$tag->openTag("h2","","","");
								$tag->output("Añadir platos a ".$restaurant->getNameMenuById($_GET['menu_id']));
							$tag->closeTag("h2");
						$tag->closeTag("div");
						$tag->openTag("table","","board-table",array('border'=>'2'));
							$tag->openTag("tr","","board-table-tr","");
								...
								$tag->closeTag("td");
							$tag->closeTag("tr");
							$restaurant->getGourmetByCifOrderByType($restaurant_id);
							if(isset($restaurant->consulta)!=null){
								foreach ($restaurant->consulta as $key => $values) {
									$tag->openTag("tr","","","");
										...
										$tag->closeTag("td");
										$tag->openTag("td","","","");
											$tag->openTag("a","","",array('href'=>'configure_menu.php?menu_id='.$_GET['menu_id'].'&gourmet_id='.$values['id_gourmet'].''));
												$tag->openTag("img","","",array('src'=>'../../site_media/img/add2.png','width'=>'30','height'=>'30'));
												$tag->closeTag("img");
											$tag->closeTag("a");
										$tag->closeTag("td");
									$tag->closeTag("tr");
