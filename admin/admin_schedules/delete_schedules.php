<?php
	...
	if($mi_sesion->getValor('id_restaurant')==TRUE){ // Controlamos que la sesión sea la del restaurante
	    ...
	    if (isset($_POST['delete_ok']) === true) { // Si el parámetro delete_ok es verdadero proseguimos con la eliminación
			...
			if($restaurant->deleteRestaurantSchedules($id_schedule,$restaurant_id)=== true){ // Eliminamos el horario del restaurante determinado

				if($restaurant->deleteSchedules($id_schedule)===true){ // Posteriormente eliminamos el horario de la tabla schedules
	...
	else if(isset($_POST['delete_nok'])){ // Si se cancela la eliminación redireccionamos a la página de horarios
...
<head>
	...
	<title>Eliminar Horarios - Portal de Administración</title>
</head>
<body>
<!--
/*************************************************************************************************************************/
...
-->
	<?php
		...
		$tag->openTag('br/','','','');
			...
			if (isset($_GET['id']) === true) { // Si recibimos un parametro por GET lo guardamos en la variable '$id_schedule' y seguimos
				
				$id_schedule=trim($_GET['id']);
			...
			if(isset($restaurant->consulta)!=null){ // Siempre y cuando la consulta sea diferente de nulo hacemos lo siguiente
				...
				foreach ($restaurant->consulta as $key => $value) { // Recorremos el array y lo almacenamos en '$value'
					...
					$form->startForm('','POST','', array('name'=>'frmempleado','class'=>'generic','enctype'=>'multipart/form-data', 'onsubmit'=>'')); // Abrimos el formulario 
						...
						// Mostramos la información del horario para que confirme su eliminación
					
						$tag->openTag('div','','none-line','');						
							$tag->openTag('label','','',array('for'=>'codigo')); 
								$tag->output('Id Schedule: '); 
							$tag->closeTag('label');						
							$input->addInput('text','id_schedule',$value['id_schedule'],array('id'=>'id_transport','placeholder'=>'','class'=>'infor','size'=>16, 'readonly'=>'readonly','required'=>'required'));
						$tag->closeTag('div');
						
						$tag->openTag('p','','','');
							$tag->output('Dia inicio: '.$restaurant->getNameById($value['day_week_start']).'.');
						$tag->closeTag('p');
						$tag->openTag('p','','','');
							$tag->output('Día final: '.$restaurant->getNameById($value['day_week_finish']).'.');
						$tag->closeTag('p');
						$tag->openTag('p','','','');
							$tag->output('Hora inicio: '.$value['time_start']);
						$tag->closeTag('p');
						$tag->openTag('p','','','');
							$tag->output('Hora final: '.$value['time_finish']);
						$tag->closeTag('p');
						$tag->openTag("br/","","","");						
						$tag->openTag('p','','','');
