<?php
	...
	if($mi_sesion->getValor('id_restaurant')==TRUE){ // Controlamos que la sesión sea la del restaurante
		...
		if(isset($_POST['delete_ok'])){ // Si el parámetro delete_ok es verdadero proseguimos con la eliminación
				...
				$speciality_id = $_POST['id_speciality'];
				...
				if(empty($errors) === true){  // Si no hay errores, llamamos a un método 'setArticle' enviando las diferentes variables recibidas por POST
					$restaurant->deleteSpecialityRestaurant($speciality_id);

					$correct[] = 'Se ha borrado correctamente.';
					header('location:../add_restaurant.php');
				}
		}else if(isset($_POST['delete_nok'])){ // Si se cancela la eliminación redireccionamos a la página de información
...
<head>
	...
	<title>Eliminar Especialidad - Portal de Administración</title>
</head>
<body>
<!--
/*************************************************************************************************************************/
...
-->
	<?php
		...
		$tag->openTag('h2','','','');
		$tag->output('Eliminar especialidad del restaurante');
		$tag->closeTag('h2');
		...
		$tag->openTag('div','','add-content-info','');
			...
			if (isset($_GET['id_speciality']) === true) { // Si recibimos un parametro por GET lo guardamos en la variable '$id_speciality' y seguimos

				$id_speciality=trim($_GET['id_speciality']);
			...
			$restaurant->getSpecialityRestaurantById($id_speciality); // Obtenemos las especialidades del restaurante cuya id la pasamos por parámetro
			...
			if(isset($restaurant->consulta)!=null){ // Siempre y cuando la consulta sea diferente de nulo hacemos lo siguiente
				...
				foreach ($restaurant->consulta as $key => $value) { // Recorremos el array y lo almacenamos en '$value'
					...
					$form->startForm('','POST','', array('name'=>'frmempleado','class'=>'generic','enctype'=>'multipart/form-data', 'onsubmit'=>'')); // Abrimos el formulario 
						...
						// Mostramos la información de la especialidad para que confirme su eliminación

						$tag->openTag('div','','none-line','');						
							$tag->openTag('label','','',array('for'=>'codigo')); 
								$tag->output('Id Tabla: '); 
							$tag->closeTag('label');						
							$input->addInput('text','id_speciality',$value['id_speciality'],array('id'=>'cif_restaurant','placeholder'=>'','class'=>'infor','size'=>16, 'readonly'=>'readonly','required'=>'required'));
						$tag->closeTag('div');
						$tag->openTag('p','','','');
							$tag->output('Especialidad: '.$value['name'].'.');
						$tag->closeTag('p');
						$tag->openTag('br/','','','');
						$tag->openTag('p','','','');
