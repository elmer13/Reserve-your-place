	...
	if($mi_sesion->getValor('id_restaurant')==TRUE){ // Controlamos que la sesión sea la del restaurante
		...
		if(isset($_POST['delete_ok'])){ // Comprobamos que se haya recibido la variable POST del 'submit'
				...
				if(empty($errors) === true){  // Si no hay errores, llamamos a un método 'setArticle' enviando las diferentes variables recibidas por POST
					...
					$restaurant->deleteGourmet($gourmet_id);
...
<head>
	...
	<title>Eliminar Gourmet - Portal de Administración</title>
</head>
<body>
<!--
/*************************************************************************************************************************/
				...
				foreach ($restaurant->consulta as $key => $value) {
					$form->startForm("","POST","", array('name'=>'frmempleado','class'=>'generic','enctype'=>'multipart/form-data', 'onsubmit'=>'')); // Abrimos el formulario 
						$tag->openTag("div","","none-line","");						
							...
							$tag->closeTag("label");						
							$input->addInput("text","id_gourmet",$value['id_gourmet'],array("id"=>"id_menu","placeholder"=>"",'class'=>'infor','size'=>16, "readonly"=>"readonly","required"=>"required"));
						$tag->closeTag("div");
						$tag->openTag("p","","","");
							$tag->output("Tipo de Gourmet: ".$restaurant->getNameByIdTypeGourmet($value['type_gourmet']).".");
						$tag->closeTag("p");
						$tag->openTag("p","","","");
						...
						$tag->openTag("p","","","");
							if($value['recomendation'] == 1){
								$tag->output("Recomendación: Si");
							}else{
								$tag->output("Recomendación: No");
							}
						$tag->closeTag("p");
						$tag->openTag("br/","","","");
						$tag->openTag("p","","","");
