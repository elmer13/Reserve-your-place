<?php
	...
	if($mi_sesion->getValor('id_restaurant')==TRUE){ // Controlamos que la sesión sea la del restaurante
		...
		if(isset($_POST['submit'])){ // Comprobamos que se haya recibido la variable POST del 'submit'
				$id_gourmet = htmlentities(trim($_POST['id_gourmet']));
				...
				if(empty($errors) === true){  
					if($restaurant->getCountsRecomendationGourmetByCif($restaurant_id) == 5 && isset($_POST['recomendation_ok'])){
						...
						$restaurant->editGourmet(array("id_gourmet"=>$id_gourmet,"type_gourmet"=>$type_gourmet, "name"=>$name,"price"=>$price, "description"=>$description,"recomendation"=>$recomendation));
						header("location:gourmet_home.php");
					}
				}
...
<head>
	...
	<title>Editar Gourmet - Portal de Administración</title>
</head>
<body>
<!--
/*************************************************************************************************************************/
...
-->
	<?php
		...
		$tag->openTag("div","contenedor","margen",""); 
			$tag->openTag("h2","","","");
				$tag->output("Editar gourmet");
			$tag->closeTag("h2");
			$tag->openTag("div","","informacion_negocio","");
			if (isset($_GET['id_gourmet']) === true) {
				$id_gourmet=trim($_GET['id_gourmet']);
			}
			$restaurant->getGourmetTypeById($id_gourmet);
			if(isset($restaurant->consulta)!=null){
				foreach ($restaurant->consulta as $key => $value) {
					$form->startForm("","POST","", array('name'=>'frmempleado','class'=>'generic','enctype'=>'multipart/form-data', 'onsubmit'=>'')); // Abrimos el formulario 
						...
						$form->addSelect("type_gourmet", $array,0,$name);
					$tag->closeTag("div");
					$tag->openTag("div","","line","");						
						$tag->openTag("label","","",array("for"=>"name_gourmet")); 
							$tag->output("Nombre Gourmet: "); 
						$tag->closeTag("label");						
						$input->addInput("text","name_gourmet",$value['name_gourmet'],array("id"=>"name_gourmet","placeholder"=>"Nombre menú",'class'=>'infor','size'=>14, "readonly"=>"","required"=>"required"));
					$tag->closeTag("div");
					$tag->openTag("div","","line","");						
						$tag->openTag("label","","",array("for"=>"price_gourmet")); 
							$tag->output("Precio Gourmet: "); 
						$tag->closeTag("label");						
						$input->addInput("text","price_gourmet",$value['price'],array("id"=>"price_gourmet","placeholder"=>"Precio menú",'class'=>'infor','size'=>14, "readonly"=>"","required"=>"required"));
					$tag->closeTag("div");
					$tag->openTag("div","","line","");
					...
					$tag->openTag("div","","line","");
						...
						$tag->closeTag("label");
						$tag->openTag("div","","","");
							if($value['recomendation'] == 1){
								$form->addInput("checkbox","recomendation_ok","Sí",array("id"=>"check_ok","placeholder"=>"",'size'=>16,'checked'=>'checked'));
									$tag->output("Si");
								...
								$form->addInput("checkbox","recomendation_ok","Sí",array("id"=>"check_ok","placeholder"=>"",'size'=>16));
									$tag->output("Sí");
								$form->addInput("checkbox","recomendation_nok","No",array("id"=>"check_nok","placeholder"=>"",'size'=>16,'checked'=>'checked'));
									$tag->output("No");
							}
						$tag->closeTag("div");
					$tag->closeTag("div");
					$tag->openTag("div","","button-align-left","");
						$input->addInput("submit","cancel","Cancelar",array('id'=>'submit','class'=>'btn'));
