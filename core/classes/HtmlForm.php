<?php	
	class HtmlForm{
		...
		public function addInput($type,$name,$value,$atributs = array()){ // Creación de los diferentes inputs teniendo en cuenta el tipo y sus caracteristicas
			...
			echo "<input type=\"".@$this->type."\" name=\"".@$this->name."\" value=\"".@$this->value."\" placeholder=\"".@$this->_atributs["placeholder"]."\" id=\"".@$this->_atributs["id"]."\" src=\"".@$this->_atributs["src"]."\" class=\"".@$this->_atributs["class"]."\" onclick=\"".@$this->_atributs['onclick']."\" step=\"".@$this->_atributs["step"]."\" size=\"".@$this->_atributs["size"]."\" ".@$this->_atributs["readonly"]." ".@$this->_atributs["checked"]." ".@$this->_atributs["required"]."/>";
		}
		...
		public function  addTextarea($name,$rows,$cols,$value,$atributs = array()){ // Creación de un textarea teniendo como atributos principales el 'rows' y el 'cols'
			...
			echo "<textarea name=\"".@$this->name."\" rows=\"".@$this->rows."\" cols=\"".@$this->cols."\" id=\"".@$this->_atributs["id"]."\" value=\"".@$this->value."\" class=\"".@$this->_atributs["class"]."\" maxlength=\"".@$this->_atributs["maxlength"]."\"  placeholder=\"".@$this->_atributs["placeholder"]."\" ".@$this->_atributs["required"].">";
		}
		...
		public function addSelect($name, $values, $multiple, $selected=null){ // Creación de un select pasandole como parametros sus caracteristicas y un array de valores predefinidos
			$this->value=$values;
			$this->name=$name;
			...
			var_dump($this->value);
	             if($this->selected == $this->value[$values]){
	                $sel = 'selected="selected"';
	             }else{
	                $sel='';
	             }
				echo "<option ". $sel ." value='".$values."'>".$this->value[$values]."</option>"; // Mostramos las diferentes opciones del select
			}
	    ...
	    public function addcheckradio($type,$name,$values,$selected=0,$id,$class){
	        ...
	        while(list($val,$l)=each($this->value)){    
	            ...
	            echo "<label><input id='".$this->id."' type='".$this->type."' name='".$this->name."' value='".$val."'".$this->check."".$this->value[$val]."</label>";
	            $c++;
	        }
	    }
