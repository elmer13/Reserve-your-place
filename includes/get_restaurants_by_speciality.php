<?php
	...
	// Archivo llamado desde ajax para filtrar la búsqueda de restaurantes por especialidades
    ...
    function buscar($b){
		...
		include_once '../core/init.php';  // Incluimos el archivo init.php que contiene las instancias de las diferentes clases 
      	...
      	$restaurant->getSpecialityByTypeSpeciality($b); // Obtenemos la información del restaurante teniendo en cuenta el tipo de especialidad seleccionada en la variable '$b'
		...
		if(isset($restaurant->consulta)!=null){  // Si la consulta es diferente de nulo mostramos los resultados
				...
				$tag->openTag('div','','producto','');
					...
					$tag->openTag('div','','informacion','');				
						...
						$tag->openTag('div','','campo3','');
							...
							if($restaurant->getSpecialityById($values['id_speciality'])){
							
								$tag->output('Especialidad: '.$restaurant->getSpecialityById($values['id_speciality']));  // Obtenemos una especialidad del restaurante aleatoriamente				
            ...
            $tag->output('No se han encontrado resultados para <b>"'.$restaurant->getSpecialityById($b).'"</b>'); 
