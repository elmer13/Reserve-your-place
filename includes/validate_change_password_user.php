<?php    
    ...
    // Este fichero valida el cambio de contraseña mediante los datos de un formulario recogido desde la parte del usuario
    ...
    if(isset($_POST['submit1'])){ // Comprobamos que se ha pulsado el submit
            ...
            }else if($bcrypt->verify($_POST['current_password'], $userData['password']) === true){ // Realizamos una llamada al método Verify para que nos compruebe la contraseña actual
        ...
        if(empty($_POST) === false && empty($errors) === true){ // Siempre y cuando los datos enviados por POST no sean vacios y no haya errores hacemos el cambio de contraseña
                ...
                $user->change_password($userData['id_user'], $_POST['password']);
