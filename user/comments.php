...
<head>
	...
	<title>Mis Comentarios - Portal del Usuario</title>
</head>
<body>
<!--
/*******************************************************************************************************************/
		...
		$tag->openTag('div','contenedor','',''); 
			...
			$tag->openTag('div','contenedor-table','','');
				...
				$tag->openTag('h2','','','');
					...
					$tag->closeTag('h2');
					if(($user->getCountReservationsByUser($user_id)) != 0){

						$input->addInput('button','new-comment','Añadir Comentario',array('id'=>'new-comment','placeholder'=>'','class'=>'btn4','onclick'=>"javascript:location.href='user_comments/add_comment.php';")); // // Añadimos el enlace para añadir comentarios

					}
					$tag->closeTag('div');

					// Creamos una tabla para mostrar los detalles de la valoración que ha realizado el usuario
					$tag->openTag('table','','board-table',array('border'=>'2'));
						...
						$tag->openTag('tr','','board-table-tr','');
							...
							$tag->closeTag('td');
							$tag->openTag('td','','','');
								$tag->output('Nota media');
							$tag->closeTag('td');
						$tag->closeTag('tr');

						$user->getRatingsByUser($user_id); // Obtenemos los datos de la valoración que ha realizado el usuario teniendo en cuenta su ID
