...
<head>
	...
	<title>Confirmar Recuperación - Portal del Usuario</title>
</head>
<body>	
<!--
/*******************************************************************************************************************/
...
-->
	<?php
		...
		if (isset($_POST['email']) === true && empty($_POST['email']) === false) { // Comprobamos que el dato recibido por POST sea diferente de vacio
			...
			if ($user->getUserByEmail($_POST['email']) === true){ // Posteriormente comprobamos si el email pertenece a un usuario  previamente registrado

				$user->confirm_recover($_POST['email']); // Por último llamamos al método para confirmar la recuperación de contraseña

				include_once '../includes/send_email_recover_password_user.php'; // Enviamos el email de confirmación al usuario 
