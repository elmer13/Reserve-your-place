...
<head>
	...
	<title>Activación - Portal del Usuario</title>
</head>
<body>
<!--
/*******************************************************************************************************************/
...
-->
	<?php
		...
		if (isset ($_GET['email'], $_GET['email_code']) === true) { // Cogemos los diferentes parametros recibidos por GET
			...
			if($user->getUserByEmail($email) === false) { // Comprobamos si el email pertenece a un usuario previamente registrado

                $errors[] = 'Lo sentimos, pero no hemos podido encontrar la dirección de correo.';

            } else if ($user->activate($email, $email_code) === false) { // Llamamos al método que se encargará de activar la cuenta a medida que comprobamos que no haya surgido algún problema
			...
			if(empty($errors) === false){  // Si ha surgido un imprevisto lo notificamos
				...
				echo '<h3>Gracias, hemos activado su cuenta. Usted sera redirigido al inicio de sesión, o si gusta presione directamente <a href="../user_access.php">aquí</a>.</h3><br/>';
